...
- **2026-07-19** — **lib/api seam rolled out to 4 high-traffic domains** (~98 call sites across 26 renderer files): `lib/api/media.ts` (MediaImage, attachmentUtils, Chat, FileBlock), `lib/api/scheduler.ts` (ScheduledModal, MyWorkSurface, EquityResearch — scheduled-research `sr*` + cron; `appLaunch*` stays on the global pending an app-launcher domain), `lib/api/memory.ts` (you/*, LearningSurface, CockpitSurface, ingestApply, useLocalCommands), `lib/api/chat.ts` (Chat screen hooks, SPS assistant slice, ActiveWorkSurface, useEquityRun, ChatSurface, SidebarRecents, CockpitSurface, ControlCenterOverview). Two refinements to the seed pattern: (1) wrappers spread `...args: Parameters<…>` so call **arity is forwarded exactly** — the seed's explicit-param style turned `fn(id)` into `fn(id, undefined)` and tripped strict `toHaveBeenCalledWith` assertions; (2) method lists are **consumer-driven** (no speculative wrappers — e.g. `onChatCheckpoint`, `getSchedulerConfig`, `updateMemoryEntry`, `rollbackLearningProposal` have no renderer consumers and are not wrapped). Bridge-presence guards (`if (!api?.method) return;` in SidebarRecents/ChatSurface/CockpitSurface/FileBlock/ingestApply) were dropped as vestigial — the standalone app they served is removed and the preload parity test guarantees the methods; verified empirically (CockpitSurface test mocks neither `listSessions` nor `readFocus` and stays green). `sps` bridge deliberately NOT migrated (1103-line bridge, ~150 call sites, heterogeneous `?.` call sites) — it goes surface-by-surface with the SpsAgent decomposition track. Consumer inventories must be swept multiline-safe (`\.method\(`), not `window\.hermesAPI\.method` — alias and chained call sites are otherwise missed (2 found post-commit this way). Commits `a88c9f3b`..`a803eee3`.
