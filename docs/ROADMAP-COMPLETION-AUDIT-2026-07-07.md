...
| 3.4 Telegram mobile client                      | `src/main/mobile-workspace-skill.ts`, `src/main/mobile-workspace-intake.ts`, `src/main/control-server.ts`, `scripts/mobile-task-live-smoke.mjs`, `tests/mobile-workspace-skill.test.ts`, `tests/mobile-workspace-intake.test.ts`, `tests/control-server.test.ts`, `tests/mobile-task-live-smoke-script.test.ts`. | Guarded local mobile task capture and explicit local live-smoke harness proven; real Telegram roundtrip pending. |
...
- run `HERMES_MOBILE_TASK_LIVE=1 node scripts/mobile-task-live-smoke.mjs --write` against a reachable
  local control server and confirm it writes exactly one review-first SPS task row;
